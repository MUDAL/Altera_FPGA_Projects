#Python code to generate random test vectors

import string
import random

NUM_OF_BITS = 12
BITS_IN_NIBBLE = 4
LEN = NUM_OF_BITS / BITS_IN_NIBBLE
NUM_BASE = 2
TESTCASES = 300
test_vector_lst = []

testcases_file        = open("../file/lcd/testcases.txt",'w')
expected_outputs_file = open("../file/lcd/expected_outputs.txt",'w')
status_reports_file   = open("../file/lcd/status_reports.txt",'w')

status_reports_file.close()

for i in range(TESTCASES):
    test_vector_bin = ''.join(random.choices(string.digits[0:2], k = NUM_OF_BITS - 2))
    test_vector_bin = "00" + test_vector_bin
    test_vector_int = int(test_vector_bin, NUM_BASE)
    test_vector_hex = format(test_vector_int, 'X')

    vector_len = len(test_vector_hex)
    len_diff = int(LEN - vector_len)
    if len_diff > 0:
        test_vector_hex = '0' * len_diff + test_vector_hex

    test_vector_lst.append(test_vector_hex)
    test_vector_lst.append('\n')

testcases_file.writelines(test_vector_lst)
expected_outputs_file.writelines(test_vector_lst)

testcases_file.close()
expected_outputs_file.close()
